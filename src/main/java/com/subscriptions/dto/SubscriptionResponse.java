...
public class SubscriptionResponse {
    ...
    public static SubscriptionResponse from(Subscription s) {
        return SubscriptionResponse.builder()
            .id(s.getId()).customerId(s.getCustomerId())
            .productId(s.getProductId()).quantity(s.getQuantity())
            .frequency(s.getFrequency()).status(s.getStatus())
            .price(s.getPrice()).discountPercent(s.getDiscountPercent())
            .nextDeliveryDate(s.getNextDeliveryDate())
            .lastDeliveryDate(s.getLastDeliveryDate())
            .skipsRemaining(s.getSkipsRemaining())
